...
The artifact syntax is `give <artifactId> [mainPropId] [<appendPropId>[,<times>]]...`. `x<amount>` and `lv<level>` 使用它时，注意它使用0-20的级别来匹配游戏中显示的数字，而不是1-21，但尽管如果你给它输入`v21`，它也不会报错。
...
去设置BP等级: `prop <bplevel|bp|battlepass> <level>`
